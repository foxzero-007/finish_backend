...
client = pymongo.MongoClient(host="127.0.0.1", port=27017,username="root",password="chq2549603631")
db = client.finishFace
...
def analyze():
	...
	if tf_version == 1:
		with graph.as_default():
			...
			save_obj = representWrapper(req,trx_id)
	elif tf_version == 2:
		resp_obj = analyzeWrapper(req, trx_id)
		save_obj = representWrapper(req, trx_id)
	...
	#---------------------------
	toc = time.time()
	resp_save = copy.deepcopy(resp_obj)
	resp_save["instance_1"]["img_source"] = req["img"]
	resp_obj["trx_id"] = trx_id
	resp_obj["seconds"] = toc-tic
	collection.insert_one(resp_save["instance_1"])
	return resp_obj, 200
...
def analyzeWrapper(req, trx_id = 0):
	...
	#print(resp_obj)
	# print(resp_obj)
	# collection.insert_one(resp_obj)
	return resp_obj
